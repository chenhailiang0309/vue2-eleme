        ...
        <transition name="showlist" v-show="category">
          <section v-show="sortBy == 'food'" class="category_container sort_detail_type">
            ...
            <section class="category_right">
              <ul>
                <li v-for="(item,index) in categoryDetail" :key="item.id" v-if="index" class="category_right_li">
                  <span>{{item.name}}</span>
          ...
          <transition name="showlist">
            <!-- <section v-show="sortBy=='activity'" class="sort_detail_type filter_container">
              <section style="width: 100%;">
                ...
                <ul class="filter_ul">
                  <li v-for="item in Delivery" :key="item.id" @click="" class="filter_li">
...
export default {
  ...
  methods: {
    ...
    // 点击顶部三个选项，展示不同的列表，选中当前选项进行展示、同时收回其它选项
    chooseType(type) {
      console.log(type)
      if (this.sortBy !== type) {
        this.sortBy = type;
        // food选项中头部标题发生变化，需特殊处理
        if (type == 'food') {
          this.foodTitle = '分类';
        } else {
          // 再次点击相同选项时收回列表
    ...
    foodDelivery() {
      this.$http({
          url: 'http://cangdu.org:8001/shopping/v1/restaurants/delivery_modes',
          type: 'post',
          data: {
            latitude: this.latitude,
            longitude: this.longitude
    ...
    foodActivity() {
      this.$http({
          url: 'http://cangdu.org:8001/shopping/v1/restaurants/activity_attributes',
          type: 'post',
          data: {
            latitude: this.latitude,
            longitude: this.longitude
